...
(function () {
  ...
  let layoutIndex = 0;
  let photoCount = 15; // 現在の写真枚数（10〜40）
  /* ════════════════════════════════════════
  ...
  function generateLayout(count, styleIndex) {
    ...
    /* 行数を枚数に応じて動的に決定 */
    let ROW_COUNT = 3;
    if (count > 18 && count <= 32) ROW_COUNT = 4;
    else if (count > 32) ROW_COUNT = 5;

    /* キャンバスの空き領域から最大サイズを逆算 (干渉しない範囲で最大化) */
    const maxInRow = Math.ceil(leftCount / ROW_COUNT);
    const maxW_pct = 43 / maxInRow; // 1列の幅約43%
    const maxH_pct = 87 / ROW_COUNT; // 利用可能高さ87%

    // 12%ほどのマージンを取り、それをベースサイズとする
    const sw = maxW_pct * 0.88;
    const sh = maxH_pct * 0.88;
  ...
  /* 装飾オブジェクトに固有Idを付与 */
  DECORATIONS.forEach((d, i) => { if(!d.id) d.id = `deco-${i}`; });
  ...
  let decoOverrides = {}; // { decoId: { x, y, scale } } — 装飾オブジェクトの位置/サイズ上書き
  let annotations = []; // [{id, x, y, text, colorIdx, r}] — ユーザー注釈
  let _annIdCtr = 0;
  ...
  async function saveState() {
    if (document.querySelector(".template-canvas.exporting")) return;
    try {
      const db = await dbPromise();
      const tx = db.transaction(STORE_NAME, "readwrite");
      
      const textsData = TEXTS.map(t => {
        const el = document.getElementById(t.id);
        if(!el) return t;
        return {
          ...t,
          html: el.innerHTML,
          color: el.style.color || t.color,
          x: parseFloat(el.style.left) || t.x,
          y: parseFloat(el.style.top) || t.y,
          size: parseFloat(el.style.fontSize) || t.size
        };
      });
      ...
      tx.objectStore(STORE_NAME).put(state, "albumState");
    } catch(err) {
  ...
  async function loadState() {
    try {
      ...
      const req = tx.objectStore(STORE_NAME).get("albumState");
      
      req.onsuccess = () => {
        const state = req.result;
        if (state) {
          photoCount = state.photoCount || 15;
          layoutIndex = state.layoutIndex || 0;
          slotOverrides = state.slotOverrides || {};
          zOverrides = state.zOverrides || {};
          slotJitter = state.slotJitter || {};
          decoOverrides = state.decoOverrides || {};
          annotations = state.annotations || [];
          
          if(state.texts) {
            state.texts.forEach(savedT => {
              const baseT = TEXTS.find(x => x.id === savedT.id);
              if (baseT) Object.assign(baseT, savedT);
            });
          }
          if (state.photos) Object.assign(photos, state.photos);
  ...
  function startObserver() {
    const ob = new MutationObserver(scheduleSave);
    ob.observe(templateEl, { childList: true, attributes: true, subtree: true, characterData: true });
  }
  ...
  function init() {
    ...
    document.getElementById("btn-bulk").addEventListener("click", () => bulkInput.click());
    ...
    document.getElementById("btn-count-minus").addEventListener("click", () => changePhotoCount(-1));
    document.getElementById("btn-count-plus").addEventListener("click", () => changePhotoCount(+1));

    bulkInput.addEventListener("change", handleBulkUpload);
    fileInput.addEventListener("change", handleSingleUpload);
    document.getElementById("btn-add-note").addEventListener("click", addAnnotation);
  ...
  function buildCanvas() {
    ...
    hdr.innerHTML = `
     ...
     <span class="ch-comment" style="white-space: nowrap;"># 狩野ゼミ HuntField 2026</span>
     <span style="font-family:'Fira Code', monospace; font-size:7px; color:rgba(0, 200, 170, 0.4); margin-left:auto; letter-spacing:0.3px; white-space:nowrap; display:inline-block; line-height:1; padding-top:2px;">[Hex:48 75 6e 74 46 69 65 6c 64 20 32 30 32 36]</span>
    ...
    /* ── ユーザー注釈 ── */
    annotations.forEach((a) => createAnnotation(a));
  }
  ...
  function createDeco(d) {
    ...
    if (node) {
      if (!d.id) d.id = `deco-${Math.random().toString(36).substr(2, 6)}`;
      node.id = d.id;
      
      let ov = decoOverrides[d.id] || { x: d.x, y: d.y, scale: 1.0 };
      
      if (d.w) node.style.width = d.w + "%";
      if (d.size) node.style.fontSize = d.size + "px";

      node.style.zIndex = d.z;
      node.style.left = ov.x + "%";
      node.style.top = ov.y + "%";
      node.style.transform = `rotate(${d.rotation || 0}deg) scale(${ov.scale})`;

      node.title = "ドラッグで移動 / スクロールで拡大縮小";
      ...
      node.addEventListener("mousedown", (e) => {
        ...
        let currentOv = decoOverrides[d.id] || { x: d.x, y: d.y, scale: 1.0 };
        const startXPct = currentOv.x;
        const startYPct = currentOv.y;
        
        node.classList.add("dragging");

        function onMove(ev) {
          const nx = startXPct + ((ev.clientX - startX) / canvasRect.width) * 100;
          const ny = startYPct + ((ev.clientY - startY) / canvasRect.height) * 100;
          
          decoOverrides[d.id] = { ...currentOv, x: nx, y: ny };
          node.style.left = nx + "%";
          node.style.top = ny + "%";
        }
      ...
      node.addEventListener("wheel", (e) => {
        ...
        e.stopPropagation();
        let currentOv = decoOverrides[d.id] || { x: d.x, y: d.y, scale: 1.0 };
        let newScale = currentOv.scale;
        if (e.deltaY < 0) {
          newScale += 0.05; 
        } else {
          newScale = Math.max(0.1, newScale - 0.05); 
        }
        decoOverrides[d.id] = { ...currentOv, scale: newScale };
        node.style.transform = `rotate(${d.rotation || 0}deg) scale(${newScale})`;
      }, { passive: false });
  ...
  function createSlot(s) {
    ...
    // ── 回転ハンドル ──
    const rotH = el("div", { className: "rotate-handle", title: "ドラッグで回転" });
    rotH.addEventListener("mousedown", (e) => initRotate(s.id, e));
    rotH.addEventListener("click", (e) => e.stopPropagation());
    slot.appendChild(rotH);
  ...
  /* ─────────────────────────────────
     EDITABLE TEXT
  ───────────────────────────────── */
  function applySelectionColor(div, color) {
    const sel = window.getSelection();
    if (!sel.rangeCount || sel.isCollapsed || !div.contains(sel.anchorNode)) {
  ...
  function adjustSelectionFontSize(div, delta, tInfo) {
    const sel = window.getSelection();
    if (!sel.rangeCount || sel.isCollapsed || !div.contains(sel.anchorNode)) {
      tInfo.size = Math.max(8, Math.min(120, tInfo.size + delta));
      div.style.fontSize = tInfo.size + "px";
      return;
    }
    
    document.execCommand("styleWithCSS", false, true);
    const tempUuid = "TEMP_FONT_XYZ_" + Math.random().toString(36).substring(2);
    document.execCommand("fontName", false, tempUuid);
    
    const elements = div.querySelectorAll('*');
    elements.forEach(el => {
      if (el.style.fontFamily && el.style.fontFamily.includes("TEMP_FONT_XYZ")) {
        el.style.fontFamily = el.style.fontFamily.replace(/TEMP_FONT_XYZ_[^"';\s]+/, '').replace(/,\s*$/, '').trim();
        if (!el.style.fontFamily) el.style.removeProperty("font-family");
        
        let currentSize = parseInt(window.getComputedStyle(el).fontSize) || 16;
        let newSize = Math.max(8, Math.min(120, currentSize + delta));
  ...
  function createText(t) {
    ...
    ci.addEventListener("input", (e) => {
      applySelectionColor(div, e.target.value);
    });
    toolbar.appendChild(ci);

    const sep2 = el("span", { className: "tc-divider" });
    toolbar.appendChild(sep2);

    const sizeMinus = el("button", { className: "tc-btn", title: "サイズを小さく" });
    sizeMinus.innerHTML = "A-";
    sizeMinus.addEventListener("mousedown", (e) => {
      e.preventDefault();
      adjustSelectionFontSize(div, -2, t);
    });
    toolbar.appendChild(sizeMinus);

    const sizePlus = el("button", { className: "tc-btn", title: "サイズを大きく" });
    sizePlus.innerHTML = "A+";
    sizePlus.addEventListener("mousedown", (e) => {
      e.preventDefault();
      adjustSelectionFontSize(div, 2, t);
    });
    toolbar.appendChild(sizePlus);

    const sep3 = el("span", { className: "tc-divider" });
    toolbar.appendChild(sep3);

    const moveHandle = el("div", { className: "tc-move", title: "ドラッグで移動" });
    moveHandle.innerHTML = "✥";
    moveHandle.addEventListener("mousedown", (e) => {
      if(e.button !== 0) return;
      initTextDrag(t, div, e);
    });
    ...
    div.adjustToolbar = function() {
      toolbar.style.left = "0px";
      toolbar.style.bottom = "calc(100% + 5px)";
      toolbar.style.top = "auto";
      toolbar.style.transform = "none";

      const cRect = templateEl.getBoundingClientRect();
      const tRect = toolbar.getBoundingClientRect();

      let tx = 0;
      if (tRect.left < cRect.left) {
        tx = cRect.left - tRect.left + 5;
      } else if (tRect.right > cRect.right) {
        tx = cRect.right - tRect.right - 5;
      }
      ...
      if (tx !== 0) {
        toolbar.style.transform = `translateX(${tx}px)`;
      }
    };

    div.addEventListener("mouseenter", div.adjustToolbar);
    div.addEventListener("focus", div.adjustToolbar, true);
  }

  function initTextDrag(t, div, startEvt) {
    ...
    function onMove(e) {
      t.x = startXPct + ((e.clientX - startX) / canvasRect.width) * 100;
      t.y = startYPct + ((e.clientY - startY) / canvasRect.height) * 100;
      div.style.left = t.x + "%";
      div.style.top = t.y + "%";
      if (div.adjustToolbar) div.adjustToolbar();
    }
    function onUp() {
      document.removeEventListener("mousemove", onMove);
      document.removeEventListener("mouseup", onUp);
  ...
  ───────────────────────────────── */
  const ANN_PALETTES = [
    { bg: "rgba(255,235,80,0.92)",  bd: "rgba(200,170,0,0.6)",   fg: "#5a4000" },
    { bg: "rgba(0,200,170,0.22)",   bd: "rgba(0,200,170,0.65)",  fg: "#004035" },
    { bg: "rgba(255,100,140,0.25)", bd: "rgba(255,60,110,0.6)",  fg: "#7a002a" },
    { bg: "rgba(139,92,246,0.22)",  bd: "rgba(139,92,246,0.65)", fg: "#3a006e" },
    { bg: "rgba(59,130,246,0.2)",   bd: "rgba(59,130,246,0.65)", fg: "#003068" },
  ];
  ...
  function addAnnotation() {
    ...
    const a = {
      ...
      text: "ここにテキスト",
      colorIdx: 0,
      r: parseFloat(((Math.random() - 0.5) * 8).toFixed(1)),
    };
    annotations.push(a);
    createAnnotation(a);
    setTimeout(() => {
      const textEl = document.querySelector(`#ann-${id} .annotation-text`);
      if (textEl) {
        textEl.focus();
    ...
    notify("注釈を追加 — ドラッグで移動 / テキストをクリックで編集 / ● で色変更");
  }

  function createAnnotation(a) {
    const pal = ANN_PALETTES[a.colorIdx % ANN_PALETTES.length];
    const wrap = el("div", { className: "annotation", id: `ann-${a.id}` });
    wrap.style.cssText = `left:${a.x}%;top:${a.y}%;transform:rotate(${a.r}deg);z-index:500;`;
    wrap.style.setProperty("--ann-bg", pal.bg);
    wrap.style.setProperty("--ann-bd", pal.bd);
    wrap.style.setProperty("--ann-fg", pal.fg);

    const bubble = el("div", { className: "annotation-bubble" });

    const icon = el("span", { className: "annotation-icon" });
    icon.textContent = "✎";
    bubble.appendChild(icon);

    const txt = el("div", { className: "annotation-text" });
    ...
    const colorBtn = el("div", { className: "annotation-color" });
    colorBtn.title = "クリックで色を変える";
    colorBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      a.colorIdx = (a.colorIdx + 1) % ANN_PALETTES.length;
      const p = ANN_PALETTES[a.colorIdx];
      wrap.style.setProperty("--ann-bg", p.bg);
      wrap.style.setProperty("--ann-bd", p.bd);
      wrap.style.setProperty("--ann-fg", p.fg);
    });
    bubble.appendChild(colorBtn);

    const delBtn = el("div", { className: "annotation-del" });
    delBtn.innerHTML = "&times;";
    delBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      annotations = annotations.filter((an) => an.id !== a.id);
      wrap.remove();
    });
    ...
    const tail = el("div", { className: "annotation-tail" });
    wrap.appendChild(tail);

    wrap.addEventListener("mousedown", (e) => {
      if (e.target.closest(".annotation-text, .annotation-del, .annotation-color")) return;
      initAnnotationDrag(a, e);
    });

    templateEl.appendChild(wrap);
  }

  function initAnnotationDrag(a, startEvt) {
    startEvt.preventDefault();
    startEvt.stopPropagation();
    const wrap = document.getElementById(`ann-${a.id}`);
    if (!wrap) return;
    const canvasRect = templateEl.getBoundingClientRect();
    const startX = startEvt.clientX;
    const startY = startEvt.clientY;
    ...
    function onMove(e) {
      a.x = startXPct + ((e.clientX - startX) / canvasRect.width) * 100;
      a.y = startYPct + ((e.clientY - startY) / canvasRect.height) * 100;
      wrap.style.left = a.x + "%";
      wrap.style.top = a.y + "%";
    }
    function onUp() {
  ...
  function renderPhoto(id, url) {
    ...
    img.onload = () => {
      ...
      if (slotOverrides[id]?.imgW === img.naturalWidth && slotOverrides[id]?.imgH === img.naturalHeight) {
        return; 
      }
      ...
      if (!slotOverrides[id]?.manual) {
        ...
        const baseWpx = (slotDef.w / 100) * canvasW;
        const baseHpx = (slotDef.h / 100) * canvasH;
        const areaPx = baseWpx * baseHpx;
        
        const imgAspect = img.naturalHeight / img.naturalWidth;
        
        const boxAspect = baseHpx / baseWpx;
        let targetWpx, targetHpx;

        if (imgAspect > boxAspect) {
          // 画像が枠より縦長なので、高さを枠いっぱいにして幅を縮める
          targetHpx = baseHpx;
          targetWpx = targetHpx / imgAspect;
        } else {
          // 画像が枠より横長なので、幅を枠いっぱいにして高さを縮める
          targetWpx = baseWpx;
          targetHpx = targetWpx * imgAspect;
        }

        // さらに、ターミナル枠などで maxBot がある場合は制約
        if (slotDef.maxBot) {
          const maxH_pct = slotDef.maxBot - slotDef.t;
          const maxH_px = (maxH_pct / 100) * canvasH;
          if (targetHpx > maxH_px) {
            targetHpx = maxH_px;
            targetWpx = targetHpx / imgAspect;
          }
        }
        
        const newW_pct = (targetWpx / canvasW) * 100;
        const newH_pct = (targetHpx / canvasH) * 100;

        slotOverrides[id] = slotOverrides[id] || {};
        
        if (Math.abs((slotOverrides[id].w || 0) - newW_pct) < 0.01 && 
            Math.abs((slotOverrides[id].h || 0) - newH_pct) < 0.01 && 
            slotOverrides[id].l !== undefined) {
             slotOverrides[id].imgW = img.naturalWidth;
             slotOverrides[id].imgH = img.naturalHeight;
             return;
        }

        slotOverrides[id].w = newW_pct;
        slotOverrides[id].h = newH_pct;
        
        // 中心を起点として伸縮させるためTop/Leftを調整
        const dw = newW_pct - slotDef.w;
        const dh = newH_pct - slotDef.h;
        slotOverrides[id].l = slotDef.l - (dw / 2);
        slotOverrides[id].t = slotDef.t - (dh / 2);
        
        slotOverrides[id].imgW = img.naturalWidth;
        slotOverrides[id].imgH = img.naturalHeight;

        // ジッターを加味してスタイル更新
        const jt = slotJitter[id] || { dl: 0, dt: 0, dr: 0 };
        slot.style.width = newW_pct + "%";
        slot.style.height = newH_pct + "%";
        slot.style.left = (slotOverrides[id].l + jt.dl) + "%";
        slot.style.top = (slotOverrides[id].t + jt.dt) + "%";
      }
    };
  ...
  /* スロットの回転 */
  function initRotate(slotId, startEvt) {
    startEvt.preventDefault();
    startEvt.stopPropagation();
    const slot = document.getElementById(`slot-${slotId}`);
    if (!slot) return;
    ...
    const slotDef = currentLayout.slots.find(s=>s.id===slotId);
    const jt = slotJitter[slotId] || { dr: 0 };
    const initialRot = (slotDef ? slotDef.r : 0) + jt.dr;
    const currentRot = slotOverrides[slotId]?.r !== undefined ? slotOverrides[slotId].r : initialRot;

    // Pointer start angle relative to center
    const startAngle = Math.atan2(startEvt.clientY - centerY, startEvt.clientX - centerX) * (180 / Math.PI);

    function onMove(e) {
      const currentMouseAngle = Math.atan2(e.clientY - centerY, e.clientX - centerX) * (180 / Math.PI);
      let dr = currentMouseAngle - startAngle;
      let newRot = currentRot + dr;
      
      slotOverrides[slotId] = slotOverrides[slotId] || {};
      slotOverrides[slotId].r = newRot;
      
      slot.style.transform = `rotate(${newRot}deg)`;
      slot.style.setProperty("--rot", `${newRot}deg`);
    }
